...
for nr in range(1, 6):
	data = pd.read_csv('CjPflA/contact-points-distance-measurements-model'+str(nr)+'.tsv', sep='\t', names=['i', 'j', 'dist'])
	...
	s = [((5-.001)*((x-mn)/(mx-mn)))+.001 for x in data['dist']]
	...
	plt.ylim(788, 0)
	plt.xlim(0, 788)
	ax.set_aspect(788/788)
	plt.tight_layout()
	plt.savefig('contact-plots/CjPflA-contact-plot-model'+str(nr), dpi=300)
	plt.show()
